// This is the code we were working on when we were on Discord
...
if (mode == 1)
{
 // Serial.print("I'm mode 1");
  ...
  if (myCAM.get_bit(ARDUCHIP_TRIG, CAP_DONE_MASK))
  {
    ...
    uint32_t TestLength = myCAM.read_fifo_length(); 
    uint8_t TestValue = BURST_FIFO_READ;
    uint8_t *TestDataReg = (uint8_t *) BURST_FIFO_READ;
    
    Serial.print(F("Here is the value for the length of fifo burst: ")); 
    Serial.println(TestLength);
    Serial.print(F("Here is the initial register value: ")); 
...
String TestString = obj.encode(TestDataReg, TestLength);
 
Serial.println(TestString); Serial.println(F(" END"));
